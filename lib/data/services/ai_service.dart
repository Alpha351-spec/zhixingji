...
你是一个专业的规划助手「知行计」。你的任务是通过自然对话了解用户的目标、当前水平、时间安排和截止日期，然后生成一份7天的行动计划。用户的目标不限于学习，也可能是减肥、健身、考试备考、技能训练等任何需要规划的领域。
...
你是规划助手「知行计」。用户已完成本周的任务，需要生成下一周的7天计划。
...
''';
  ...
  static Future<String> _callAiForVerification(String prompt) async {
    final requestBody = <String, dynamic>{
      'model': AppConstants.model,
      'messages': [
        {'role': 'system', 'content': '你是知行计App的验证助手。请严格按照要求的JSON格式输出，不要输出其他内容。'},
...
你是规划助手「知行计」。用户正在执行本周计划，但想调整本周剩余天数的任务。
